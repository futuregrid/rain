...
class IMRegisterServerXcat(object):
    ...
    def process_client(self, connstream, fromaddr):
        ...
        #PATH of the image on /install/netboot/: 
        #self.cleanTftpboot()
    ...
    def handle_image(self, image, connstream):
        ...
        if status != 0:
            ...
            self.errormsg(connstream, msg)
            self.cleanInstallNetboot()  #REMOVE DIRECTORY in /install/netboot
            return False
        ...
        if status_cp != 0:
            ...
            self.errormsg(connstream, msg)
            #we have to umount the image before we clean
    ...
    def customize_ubuntu_img(self):
        ...
        if(self.machine == "minicluster"):
            ...
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_minicluster/pbs_environment -O ' + \
                                  self.path + '/rootimg/var/spool/torque/pbs_environment'))          
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_minicluster/server_name -O ' + \
...
'''
            ...
            statuslist.append(self.runCmd('wget ' + self.http_server + '/conf/authusers -O ' + \
                                  self.path + '/rootimg/etc/authusers'))
            
            self.logger.info('Torque for India')
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_india/pbs_environment -O ' + \
                                  self.path + '/rootimg/var/spool/torque/pbs_environment'))
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_india/server_name -O ' + \
                                  self.path + '/rootimg/var/spool/torque/server_name'))
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_india/prologue -O ' + \
                                  self.path + '/rootimg/var/spool/torque/mom_priv/prologue'))
            statuslist.append(self.runCmd('wget ' + self.http_server + '/torque/config_india/epilogue -O ' + \
